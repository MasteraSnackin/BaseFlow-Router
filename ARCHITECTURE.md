...
```mermaid
graph TD
    User[User / Trader] -->|Interacts via Browser| Frontend[React Frontend]
    Frontend -->|Requests Quote| Backend[Backend API Service]
    Backend -->|Validates Price| CG[CoinGecko API]
    Backend -->|Simulates & Encodes| Provider[RPC Provider]
    Frontend -->|Submits Transaction| Wallet[User Wallet / MetaMask]
    Wallet -->|Signs & Sends| BaseNetwork[Base Blockchain]
    
    subgraph "Base Blockchain"
        Router[Router Contract]
        VenueA[Venue A (DEX)]
        VenueB[Venue B (DEX)]
        Pump[RobinPump.fun Curve]
        
        Router -->|Routes Trade| VenueA
        Router -->|Routes Trade| VenueB
        Router -->|Routes Trade| Pump
    end
```
...
sequenceDiagram
    ...
    participant BC as Blockchain (Router)
    
    U->>FE: Selects Token & Amount
    FE->>BE: POST /quote {tokenIn, tokenOut, amount}
    
    par Quote Aggregation
        BE->>BC: Call Venue A (Simulation)
        BE->>BC: Call Venue B (Simulation)
        BE->>BC: Call RobinPump (Simulation)
    end
    
    BE->>BE: Compare & Select Best Route
    BE->>FE: Return {bestVenue, minAmountOut, calldata}
    
    U->>FE: Clicks "Swap"
    FE->>U: Request Wallet Signature
    U->>BC: Submit Transaction (via Router)
    
    BC->>BC: Router transfers User tokens
    BC->>BC: Router calls Best Venue
    BC->>U: Transfer Resulting Tokens
    BC-->>FE: Transaction Receipt
    FE->>U: "Swap Complete"
```
